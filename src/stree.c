...
static void snode_clone(snode_t * snode, snode_t * clone, stree_t * clone_stree)
{
  ...
  if (snode->brate)
  {
    brate_count = (opt_clock == BPP_CLOCK_SIMPLE) ? 1 : msa_count;

    if (!clone->brate)
      clone->brate = (double *)xmalloc(brate_count * sizeof(double));
    memcpy(clone->brate, snode->brate, brate_count * sizeof(double));
  }
  ...
  if (opt_migration)
  {
    ...
    for (i = 0; i < clone_stree->inner_count; ++i)
    {
      ...
      long j;
      for (j = 0; j < snode->migbuffer[i].donors_count; ++j)
      {
        long node_index = snode->migbuffer[i].donors[j]->node_index;
        snode_t * cnode = clone_stree->nodes[node_index];
        clone->migbuffer[i].donors[j] = cnode;
      }
      clone->migbuffer[i].donors_count = snode->migbuffer[i].donors_count;
    }
  }
...
static void stree_clone(stree_t * stree, stree_t * clone)
{
  ...
  if (opt_migration)
  {
    ...
    for (i = 0; i < nodes_count; ++i)
    {
      ...
      for (j = 0; j < nodes_count; ++j)
      {
        ...
        memcpy(clone->Wsji[i][j],
               stree->Wsji[i][j],
               opt_locus_count*sizeof(double));
      }
    }
    //debug_print_wsji(clone,3);
  }
  ...
  if (opt_datefile)
  {
    if (clone->u_constraint)
      memcpy(clone->u_constraint,
             stree->u_constraint,
             stree->inner_count * sizeof(double));
    memcpy(clone->l_constraint,
           stree->l_constraint,
           stree->inner_count * sizeof(double));
  }
}

static void stree_alloc_wsji_matrices(stree_t * stree)
{
  unsigned int i,j;
  ...
  assert(stree->Wsji == NULL);
  assert(stree->old_Wsji == NULL);

  /* Create Wsji matrix as NxN matrix where row corrsponding is empty (NULL)
     and row corresponding to self or root is also NULL */
  stree->Wsji = (double ***)xmalloc((size_t)nodes_count*sizeof(double **));
  stree->old_Wsji = (double ***)xmalloc((size_t)nodes_count*sizeof(double **));
  for (i = 0; i < nodes_count; ++i)
  {
    ...
    /* the above loop covers this case only when rates are not inferred */
    if (i == stree->root->node_index)
    {
      stree->Wsji[i] = NULL;
      stree->old_Wsji[i] = NULL;
    ...
    stree->Wsji[i] = (double **)xmalloc((size_t)nodes_count*sizeof(double *));
    stree->old_Wsji[i] = (double**)xmalloc((size_t)nodes_count*sizeof(double*));
    for (j = 0; j < nodes_count; ++j)
    {
      if (!opt_est_geneflow && !opt_mig_bitmatrix[i][j])
      {
      ...
      stree->Wsji[i][j] = (double *)xmalloc((size_t)opt_locus_count *
                                            sizeof(double));
      stree->old_Wsji[i][j] = (double *)xmalloc((size_t)opt_locus_count *
                                                sizeof(double));
    }
  }
}
...
stree_t * stree_clone_init(stree_t * stree)
{
  ...
  for (i = 0; i < nodes_count; ++i)
  {
    ...
    if (opt_migration)
    {
      ...
      if (snode->old_C2ji)
        x->old_C2ji = (double *)xcalloc((size_t)opt_locus_count, sizeof(double));
      else
        x->old_C2ji = NULL;

      if (snode->C2ji)
        x->C2ji = (double *)xcalloc((size_t)opt_locus_count, sizeof(double));
      else
        x->C2ji = NULL;
    }
  }
  ...
  if (opt_migration)
  {
    ...
    /* Create Wsji matrix as NxN matrix where row corrsponding is empty (NULL)
       and row corresponding to self or root is also NULL */
    stree_alloc_wsji_matrices(clone);
  }
                                         ...
                                         double *l_constraint)
{
  ...
  stree_init_tau_recursive_constraint(stree, node->left, prop, thread_index, u_constraint, l_constraint);
...
static void mscm_link_thetas(stree_t* stree)
{
  ...
  for (i = 0; i < stree->tip_count; ++i)
  {
    ...
    long parent = pparent->node_index, sib = pparent->left->node_index;
...
static void init_w_stepsize()
{
  long i;
  double w_eps_default = 0.1;

  if (opt_finetune_mrate_mode == 1) return;

  if (opt_finetune_mrate_mode != 2)
    fatal("Internal error: invalid opt_finetune_mrate_mode (%ld)",
          opt_finetune_mrate_mode);

  assert(opt_finetune_mrate_mode == 2);
  assert(!opt_est_geneflow);  /* doesn't affect anything at this point */

  for (i = 0; i < opt_migration_count; ++i)
    if (opt_finetune_migrates[i] < 0)
      opt_finetune_migrates[i] = w_eps_default;

  if (opt_mig_vrates_exist)
  {
    for (i = 0; i < opt_migration_count; ++i)
      if (opt_finetune_mig_Mi[i] < 0)
        opt_finetune_mig_Mi[i] = w_eps_default;
  }
}
                ...
                FILE * fp_out)
{
  ...
  if (opt_migration)
  {
    ...
    stree->Wsji = NULL;
    stree->old_Wsji = NULL;
    stree_alloc_wsji_matrices(stree);
  ...
  if (opt_clock != BPP_CLOCK_GLOBAL)
  {
    for (i=0; i < stree->tip_count+stree->inner_count+stree->hybrid_count; ++i)
    {
      ...
      snode->brate = (double *)xmalloc((size_t)brate_count*sizeof(double));
    }
  }
                                 ...
                                 double * a1, double * b1)
{
  /* This returns the approximate posterior for the expected waiting time for a Poisson event,
    ...
    It is also the approximate posterior for theta=4Nu, gamma G(a1,b1), under the gamma prior
    theta~G(a,b) and data of k coalescent events over time period T (but call the function with
    2*T).
    ziheng-2024.1.31
  */
  int gamma_method = 0;
                                  ...
                                  long thread_index)
{
  ...
  assert(coal_sum == snode->coal_count_sum);
  ...
  {
    /* ziheng-2024.1.31 */
    // b1 = opt_theta_beta * a1 * (a1+1) / (opt_theta_alpha*(opt_theta_alpha+1) + C2h_sum*opt_theta_beta);
  ...
  if (opt_theta_prior == BPP_THETA_PRIOR_GAMMA)
  {
    ...
    if (debug_gibbs) {
      long k = coal_sum;
      double a = opt_theta_alpha, b = opt_theta_beta, C2 = C2h_sum, lnacc;
      if (opt_theta_prop & BPP_THETA_PROP_MG_GAMMA) {
        /* proposal ratio & posterior ratio */
        lnacc = (a1 - 1) * log(oldtheta / newtheta) - b1 * (oldtheta - newtheta);
        lnacc += (a - 1 - k) * log(newtheta / oldtheta) - b * (newtheta - oldtheta) - C2 * (1 / newtheta - 1 / oldtheta);
        printf("%6ld%3ld %-2s: kT= %4ld %9.6f a1b1 = %6.1f %8.1f  means= %8.5f %8.5f %8.5f reject %8.5f %8.5f %8.3f = %8.3f\n",
          (debug_gibbs_times - 1) / 3, (debug_gibbs_times - 1) % 3, snode->label, k, C2, a1, b1, a / b, C2 / k, a1 / b1,
        ...
        lnacc += (a - 1 - k) * log(newtheta / oldtheta) - b * (newtheta - oldtheta) - C2 * (1 / newtheta - 1 / oldtheta);
        printf("%6ld%3ld %-2s: kT= %4ld %9.6f a1b1 = %6.1f %8.1f  means= %8.5f %8.5f %8.5f reject %8.5f %8.5f %8.3f = %8.3f\n",
          (debug_gibbs_times - 1) / 3, (debug_gibbs_times - 1) % 3, snode->label, k, C2, a1, b1, a / b, C2 / k, b1 / (a1 - 1),
          oldtheta, newtheta, lnacceptance, lnacc);
      }
    }
                               ...
                               long thread_index)
{
  ...
  /* TODO: We can store C2h in the gtree_update_logprob_contrib function */
  if (opt_linkedtheta)
  {
                         ...
                         long * ft_round_slide)
{
    ...
    if (snode->theta >= 0 && snode->has_theta && !snode->linked_theta)
    {
          ...
          legacy_rndu(thread_index_zero) < opt_theta_slide_prob)
      {
        ...
        accepted = propose_theta_slide(stree,
                                       ...
                                       snode,
                                       thread_index_zero);
        if (opt_finetune_theta_mode == 3)
        {
          ft_round_slide[snode->theta_step_index]++;
          RMEAN_UPDATE(g_pj_theta_slide[snode->theta_step_index],
                       ft_round_slide[snode->theta_step_index],
                       accepted);
        }
        else
        {
          ...
          {
            slide_tip_count++;
            slide_tip_accept += accepted;
          }
          else
          {
            slide_inner_count++;
            slide_inner_accept += accepted;
          }
        }
      }
      else
      {
        ...
        if (opt_finetune_theta_mode == 3)
        {
          ft_round_gibbs[snode->theta_step_index]++;
          RMEAN_UPDATE(g_pj_theta_gibbs[snode->theta_step_index],
                       ft_round_gibbs[snode->theta_step_index],
                       accepted);
        }
        if (i < stree->tip_count)
  ...
  if (opt_finetune_theta_mode < 3)
  {
    ...
    {
      assert(slide_inner_count+slide_tip_count > 0);
      assert(gibbs_inner_count+gibbs_tip_count == 0);
    }
  ...
  switch (opt_finetune_theta_mode)
  {
    case 1:
      ...
      {
        ft_round_slide[0]++;
        ratio = (slide_tip_accept+slide_inner_accept) / (double)total_slide;
        RMEAN_UPDATE(g_pj_theta_slide[0],ft_round_slide[0],ratio);
      }

      if (total_gibbs)
      {
        ft_round_gibbs[0]++;
        ratio = (gibbs_tip_accept+gibbs_inner_accept) / (double)total_gibbs;
        RMEAN_UPDATE(g_pj_theta_gibbs[0],ft_round_gibbs[0],ratio);
      }
    ...
    case 2:
      ...
      {
        ft_round_slide[0]++;
        ratio = slide_tip_accept / (double)slide_tip_count;
        RMEAN_UPDATE(g_pj_theta_slide[0],ft_round_slide[0],ratio);
      }
      ...
      {
        ft_round_slide[1]++;
        ratio = slide_inner_accept / (double)slide_inner_count;
        RMEAN_UPDATE(g_pj_theta_slide[1],ft_round_slide[1],ratio);
      }

      if (gibbs_tip_count)
      {
        ft_round_gibbs[0]++;
        ratio = gibbs_tip_accept / (double)gibbs_tip_count;
        RMEAN_UPDATE(g_pj_theta_gibbs[0],ft_round_gibbs[0],ratio);
      }
      ...
      {
        ft_round_gibbs[1]++;
        ratio = gibbs_inner_accept / (double)gibbs_inner_count;
        RMEAN_UPDATE(g_pj_theta_gibbs[1],ft_round_gibbs[1],ratio);
      }
    ...
    for (k = 0; k < x->mb_count; ++k)
    {
      ...
      for (j = 0; j < total_nodes-1; ++j)
      {
            ...
            y->tau <= tstart && y->parent->tau >= tend)
        {
          ...
          /* add pop y into the list of donors */
          xm[k].donors[dcount++] = y;
        }
      }
      xm[k].donors_count = dcount;
      tstart = tend;
    }
  }
  ...
  for (i = 0; i < gtree->tip_count + gtree->inner_count; ++i)
  {
    ...
    for (j = 0; j < x->mi->count; ++j)
    {
      ...
      for (k = 0; k < paffected_count; ++k)
        if (affected[k]->flag & SN_AFFECT && (x->mi->me[j].source == affected[k] || x->mi->me[j].target == affected[k]))
          break;
                                   ...
                                   long thread_index)
{
  ...
  *ret_mig_reject = 0;
  for (i = locus_start; i < locus_start+locus_count; ++i)
  {
    ...
    unsigned int new_affected_count = paffected_count;
    for (j = 0; j < paffected_count; ++j)
    {
      unsigned int kk;
      for (kk = 0; kk < stree->tip_count+stree->inner_count; ++kk)
      {
        if (stree->nodes[kk]->mark[thread_index]) continue;

        if (opt_mig_bitmatrix[kk][affected[j]->node_index] || opt_mig_bitmatrix[affected[j]->node_index][kk])
        {
          affected[new_affected_count++] = stree->nodes[kk];
        }
      }
    }
    ...
    for (j = 0; j < paffected_count; ++j)
    {
      ...
      double xtmp = gtree_update_logprob_contrib_mig(affected[j],
                                                     stree,
                                                     gtree[i],
                                                     loci[i]->heredity[0],
                                                     i,
...
static double partial_C2j(snode_t * snode,
                          double heredity,
                          double oldage,
                          double minage,
                          double maxage,
                          ...
                          long thread_index)
{
  ...
  for (coalevent = snode->coalevent[msa_index]->head; coalevent; coalevent = coalevent->next)
  {
    ...
    {
      t = (t >= oldage) ?
            maxage+maxfactor*(t-maxage) : minage+minfactor*(t-minage);
    }
  ...
  double prev = sortbuffer[0];
  for (k = 1, n = snode->seqin_count[msa_index]; k < j; ++k, --n)
  {
    double t = sortbuffer[k];
...
static double partial_C2j_and_Wsj(stree_t * stree,
                                  snode_t * snode,
                                  double heredity,
                                  double oldtau,
                                  double minage,
                                  ...
                                  long thread_index)
{
  ...
  if (snode->parent && !snode->mb_count)
    fatal("\nError when processing node %s", snode->label);
  ...
  */
  ...
  long epoch = 0;
  #if 0
  /* backup and reset old Wsji */
  if (snode->flag & SN_MIG_UPDATE)
  {
  ...
  /* calculate C2j */
  double tprev = migbuffer[0].time;
  for (k = 1, n = snode->seqin_count[msa_index]; k < j; ++k)
  {
    double t = migbuffer[k].time;
    if (t >= minage && t <= maxage && migbuffer[k].type != EVENT_TAU)
      t = (t >= oldtau) ?
              maxage+maxfactor*(t-maxage) : minage+minfactor*(t-minage);
    ...
    if (n > 0 && snode->parent)
      Wsji += n*(t-tprev);
      /* TODO: Add a flag to check its a target population to avoid some calculations */
    ...
    else if (migbuffer[k].type == EVENT_TAU)
    {
      if (snode->flag & SN_MIG_UPDATE)
      {
        for (i = 0; i < snode->migbuffer[epoch].donors_count; ++i)
        {
          ...
          //stree->Wsji[si][ti][msa_index] += Wsji;
          wsji_sum[si] += Wsji;
        }
      }
                        ...
                        long thread_index)
{
  ...
  if (opt_est_theta && opt_rb_theta_update)
  {
    ...
    for (j = 0; j < theta_count; ++j)
    {
      ...
      for (jj = 0; jj < totnodes; ++jj)
      {
        ...
        for (ii = 0; ii < opt_locus_count; ++ii)
        {
          ...
          double dbg_fullC2j = 0;

          long dbg_shortcut = 0;

          if (snodes[jj] != snode) 
          {
            snode_t * sibling = snode->left == snodes[jj] ? snode->right : snode->left;
            if (snodes[jj]->tau > sibling->tau)
              dbg_shortcut = 1;
          }
          else
          {
          ...
          if (snodes[jj]->flag & SN_AFFECT)
            mfactor =  (snodes[jj]->tau >= snode->tau) ? maxfactor: minfactor;
          dbg_shortC2j = snodes[jj]->C2ji[ii]*mfactor;

          C2j_new += dbg_shortC2j;
          #endif

          if (snodes[jj]->flag & SN_AFFECT)
            C2j_new += partial_C2j(snodes[jj],
                                   loci[ii]->heredity[0],
                                   oldage,
                                   minage,
                                   maxage,
      ...
      double b1 = opt_theta_beta + C2j_new;
      ...
      /* proposal ratio */
      if (opt_theta_prop == BPP_THETA_PROP_MG_GAMMA)
        lnacceptance += logPDFGamma(x->old_theta, a1_old, b1_old) -
      ...
      if (opt_debug_tau)
        printf("%-3s k C2h_new: %ld %9.6f a1 b1: %9.6f %9.6f -> %9.6f %9.6f theta: %9.5e %9.5e lna = %7.4f\n", 
          x->label, coal_count_sum, C2j_new, a1_old, b1_old, a1, b1, x->old_theta, x->theta, lnacceptance);
    ...
    if (opt_linkedtheta)
    {
      ...
      long new_theta_count = theta_count;
      for (j = 0; j < theta_count; ++j)
      {
        long jj;
        for (jj = 0; jj < totnodes; ++jj)
        {
          snode_t * x = snodes[jj];
          if (x->linked_theta == theta_list[j] && marks[x->node_index] == 0)
          {
            theta_list[new_theta_count++] = x;
            marks[x->node_index] = 1;
          }
        }
      }
      theta_count = new_theta_count;
    }
  }
    ...
    for (i = 0; i < stree->locus_count; ++i)
    {
      ...
      if (opt_est_theta)
      {
        for (j = 0; j < theta_count; ++j)
        {
          ...
          {
            if (opt_migration)
              gtree_update_logprob_contrib_mig(theta_list[j],
                                               stree,
                                               gtree[i],
                                               loci[i]->heredity[0],
                                               i,
              ...
              gtree_update_logprob_contrib(theta_list[j],
                                           loci[i]->heredity[0],
                                           i,
                                           thread_index);
          }
                            ...
                            long thread_index)
{
  ...
  /* change migration rates first */
  if (mig_method)
  {
    long kk;
    for (kk = 0; kk < opt_migration_count; ++kk)
    {
      migspec_t * spec = opt_mig_specs+kk;

      snode_t * src = stree->nodes[spec->si];
      snode_t * dst = stree->nodes[spec->ti];

      if (!((src->flag & SN_AFFECT && !(src->flag & SN_MIG_UPDATE)) ||
            (dst->flag & SN_AFFECT && !(dst->flag & SN_MIG_UPDATE)))) continue;


      for (j = 0; j < opt_locus_count; ++j)
        if (gtree[j]->migcount[spec->si][spec->ti])
         break;

      if (j == opt_locus_count) continue;
      
      /* TODO: this is too general -- even if they are affected, it could be that the migration part is outside
         the bounds */
      if (src->flag & SN_AFFECT)
        src->flag |= SN_MIG_UPDATE;
      if (dst->flag & SN_AFFECT)
        dst->flag |= SN_MIG_UPDATE;
    }
  }
  ...
  if (opt_est_theta && theta_method)
  {
    ...
    double * wsji_sum = (double *)xmalloc(totnodes*sizeof(double));
    ...
    for (j = 0; j < theta_count; ++j)
    {
      ...
      for (jj = 0; jj < totnodes; ++jj)
      {
        ...
        for (ii = 0; ii < opt_locus_count; ++ii)
        {
          ...
          if (snodes[jj]->flag & SN_AFFECT)
          {
            C2j_new += partial_C2j_and_Wsj(stree,
                                           snodes[jj],
                                           loci[ii]->heredity[0],
                                           oldage,
                                           minage,
        ...
        if (snodes[jj]->flag & SN_MIG_UPDATE)
        {
          for (ii = 0; ii < totnodes; ++ii)
          {
            if (wsji_sum[ii])
            {
              /* update migration */
              long mindex = opt_migration_matrix[ii][snodes[jj]->node_index];
              assert(mindex >= 0);
              ...
              long asj = 0;
              double bsj_old = 0;
              for (kk = 0; kk < opt_locus_count; ++kk)
              {
                asj     += gtree[kk]->migcount[spec->si][spec->ti];
                bsj_old += stree->Wsji[spec->si][spec->ti][kk];
              }

              double a1 = spec->alpha + asj;
              double b1 = spec->beta + wsji_sum[ii];
              double old_w = spec->M;
              spec->old_M = spec->M;
              spec->M = legacy_rndgamma(thread_index,a1)/b1;
              double new_w = spec->M;
              assert(spec->mark == 0);
              spec->mark = 1;
              ...
              /* proposal ratio */
              lnacceptance += logPDFGamma(old_w, a1, spec->beta+bsj_old) - logPDFGamma(new_w,a1,b1);
  ...
  if (!opt_est_theta)
  {
    old_logpr_contrib = (double *)xmalloc((size_t)paffected_count*sizeof(double));
    for (i = 0; i < paffected_count; ++i)
  ...
  if (opt_debug)
    debug_consistency(stree,gtree,"tau_mig");
  ...
  */
  ...
  else if (opt_clock == BPP_CLOCK_IND && opt_rate_prior == BPP_BRATE_PRIOR_LOGNORMAL)
  {
    ...
    */
    ...
    fatal("Internal error when calculating rates prior for clock %ld", opt_clock);
                          ...
                          long thread_index)
{
  ...
  */
  ...
  for (i = 0; i < opt_locus_count; ++i)
  {
    ...
    {
      /* if molecular,correlated or lineage rate clock then swap pmatrices */
                              ...
                              locus_t ** locus)
{
  ...
  /* TODO: get rid of mem allocation */
  unsigned int * pcount_list;
  pcount_list = (unsigned int *)xmalloc((size_t)opt_locus_count*sizeof(unsigned int));
  ...
  for (j = 0; j < stree->tip_count+stree->inner_count+stree->hybrid_count; ++j)
  {
    ...
    old_rate = node->brate[0];
    old_lograte = log(old_rate);

    double r = old_lograte + opt_finetune_branchrate *
               legacy_rnd_symmetrical(thread_index_zero);
    new_lograte = reflect(r,-99,99,thread_index_zero);
    node->brate[0] = new_rate = exp(new_lograte);
    ...
    if (opt_usedata)
    {
      for (i = 0; i < opt_locus_count; ++i)
      {
        ...
        {
          /* swap pmatrices */
          for (k = 0; k < gtree[i]->tb_count; ++k)
          {
            gnode_t * x = updatelist[k];
            x->pmatrix_index = SWAP_PMAT_INDEX(gtree[i]->edge_count,
                                               x->pmatrix_index);
          }
          ...
          gtree_return_partials(gtree[i]->root,
                                gtree[i]->travbuffer,
                                &partials_count);
          partials = gtree[i]->travbuffer;
          pcount_list[i] = partials_count;
    ...
    alpha = beta = opt_clock_alpha;
    diff = (alpha-1)*(new_lograte-old_lograte) - beta*(new_rate-old_rate);
    ...
    if (lnacceptance >= -1e-10 || legacy_rndu(thread_index_zero) < exp(lnacceptance))
    {
      /* accepted */
      accepted++;
      stree->lnprior_rates_simple += diff;
    }
    else
    {
      /* rejected */
                              ...
                              long thread_index)
{
  ...
  assert(opt_clock != BPP_CLOCK_GLOBAL && opt_clock != BPP_CLOCK_SIMPLE);
                           ...
                           snode_t ** rway)
{
  ...
  *lnacceptance += newprior - oldprior;
  ...
  #if 0
  debug_consistency(stree, gtree_list, "snl");
  #endif
...
static double migrate_gibbs(stree_t * stree, gtree_t ** gtree, migspec_t * spec)
{
  long i;
  long asj = 0;
  long msa_index;
  ...
  for (msa_index = 0; msa_index < opt_locus_count; ++msa_index)
  {
    asj += gtree[msa_index]->migcount[spec->si][spec->ti];
    bsj += stree->Wsji[spec->si][spec->ti][msa_index];
  }
  ...
  snode_t * srcnode = stree->nodes[spec->si];
  snode_t * tgtnode = stree->nodes[spec->ti];
  stree_update_mig_subpops_single(tgtnode, srcnode, old_w);

  /* update gene tree density */
  for (i = 0; i < opt_locus_count; ++i)
  {
    long ** mc = gtree[i]->migcount;

    double new_logpr_contrib = tgtnode->logpr_contrib[i];

    new_logpr_contrib -= mc[spec->si][spec->ti] * log(old_w);
    new_logpr_contrib += mc[spec->si][spec->ti] * log(new_w);

    new_logpr_contrib += stree->Wsji[spec->si][spec->ti][i]*old_w;
    new_logpr_contrib -= stree->Wsji[spec->si][spec->ti][i]*new_w;

    gtree[i]->logpr -= tgtnode->logpr_contrib[i];
    gtree[i]->logpr += new_logpr_contrib;
    tgtnode->logpr_contrib[i] = new_logpr_contrib;
  }
  return 1;
}

static long migrate_mbar_slide(migspec_t * spec)
{
  long i;
  long accepted = 0;
  double c,lnc;
  ...
  lnc = opt_finetune_migrates[spec->index] * legacy_rnd_symmetrical(thread_index_zero);
  c = exp(lnc);
...
static double migrate_slide(stree_t * stree, gtree_t ** gtree, migspec_t * spec)
{
  long i;
  double c,lnc;
  ...
  assert(opt_est_theta);
  assert(migration_valid(srcnode, tgtnode));
  assert(!spec->am);
  ...
  stree_update_mig_subpops_single(tgtnode, srcnode, rate_old);
  ...
  for (i = 0; i < opt_locus_count; ++i)
  {
    ...
    new_logpr_contrib -= mc[spec->si][spec->ti] * log(rate_old);
    new_logpr_contrib += mc[spec->si][spec->ti] * log(rate_new);

    new_logpr_contrib += stree->Wsji[spec->si][spec->ti][i]*rate_old;
    new_logpr_contrib -= stree->Wsji[spec->si][spec->ti][i]*rate_new;

    gtree[i]->logpr -= tgtnode->logpr_contrib[i];
    gtree[i]->logpr += new_logpr_contrib;

    /* logpr difference */
    lnacceptance += new_logpr_contrib - tgtnode->logpr_contrib[i];

    tgtnode->old_logpr_contrib[i] = tgtnode->logpr_contrib[i];
    tgtnode->logpr_contrib[i] = new_logpr_contrib;
  }
  #endif
  ...
  else
  {
    ...
    stree_update_mig_subpops_single(tgtnode, srcnode, rate_new);

    for (i = 0; i < opt_locus_count; ++i)
    {
      gtree[i]->logpr = gtree[i]->old_logpr;
      tgtnode->logpr_contrib[i] = tgtnode->old_logpr_contrib[i];
    }
  }
  return 0;
}
...
static double prop_mig_vrates_slide(stree_t * stree, gtree_t ** gtree, locus_t ** locus)
{
  ...
  for (i = 0; i < opt_migration_count; ++i)
  {
    ...
    for (j = 0; j < opt_locus_count; ++j)
    {
      ...
      #if 1
      lnc = opt_finetune_mig_Mi[spec->index] * legacy_rnd_symmetrical(thread_index_zero);
      c = exp(lnc);
...
void prop_migrates(stree_t * stree,
                   gtree_t ** gtree,
                   locus_t ** locus,
                   long * ft_round_gibbs,
                   long * ft_round_slide)
{
  ...
  long total_slide = 0;
  long total_gibbs = 0;
  long total_accept_slide = 0;
  long total_accept_gibbs = 0;
  double ratio;
  migspec_t * spec;
  ...
  {
    spec = opt_mig_specs+i;
    movetype = BPP_MRATE_NONE;
    
    if (!migration_valid(stree->nodes[spec->si], stree->nodes[spec->ti]))
      continue;

    /* decide on the move (gibbs or sliding window) */
    if (opt_mrate_slide_prob > 0 || opt_mig_vrates_exist)
    {
      if (spec->am || opt_mrate_slide_prob == 1)
        movetype = BPP_MRATE_SLIDE;
      else
        movetype = (legacy_rndu(thread_index) < opt_mrate_slide_prob) ?
                      BPP_MRATE_SLIDE : BPP_MRATE_GIBBS;
    }
    else
    {
      movetype = BPP_MRATE_GIBBS;
    }
    
    /* execute the move */
    if (movetype == BPP_MRATE_SLIDE)
    {
      /* sliding window */
      mt_slide = 1;
      ...
      if (opt_finetune_mrate_mode == 2)
      {
        ft_round_slide[spec->index]++;
        RMEAN_UPDATE(g_pj_mrate_slide[spec->index],ft_round_slide[spec->index],accepted);
      }
      else
      {
        /* calculate total ratio */
        total_slide++;
        total_accept_slide += accepted;
      }
    }
    else
    {
      ...
      if (opt_finetune_mrate_mode == 2)
      {
        ...
        RMEAN_UPDATE(g_pj_mrate_gibbs[spec->index],ft_round_gibbs[spec->index],accepted);
      }
      else
      {
        /* calculate total ratio */
        total_gibbs++;
        total_accept_gibbs += accepted;
      }
    }
  }
  ...
  if (opt_finetune_mrate_mode == 1)
  {
    ...
    {
      ft_round_slide[0]++;
      ratio = total_accept_slide / (double)total_slide;
      RMEAN_UPDATE(g_pj_mrate_slide[0],ft_round_slide[0],ratio);
    }
    if (mt_gibbs)
    {
      ft_round_gibbs[0]++;
      ratio = total_accept_gibbs / (double)total_gibbs;
      RMEAN_UPDATE(g_pj_mrate_gibbs[0],ft_round_gibbs[0],ratio);
    }
  }
