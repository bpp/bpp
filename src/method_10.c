...
void cmd_a10()
{
  ...
  unsigned int ** weights = (unsigned int **)xmalloc(msa_count *
                                                     sizeof(unsigned int *));
  int * ol = (int *)xmalloc(msa_count * sizeof(int));
  for (i = 0; i < msa_count; ++i)
  {
    ol[i] = msa_list[i]->length;
    msa_list[i]->original_length = ol[i];
  ...
  /* TODO CALL HERE */
  /* WE msut first link tip sequences (gene tips) to populations */
  stree_rootdist(stree,map_list,msa_list,weights,ol);
  ...
  for (i = -opt_burnin; i < opt_samples*opt_samplefreq; ++i)
  {
    ...
    if (legacy_rndu() < 0.5)
      j = prop_split(gtree,stree,locus,0.5,&param_count);
    else
      j = prop_join(gtree,stree,locus,0.5,&param_count);
    ...
    {
      mcmc_logsample(fp_mcmc,i+1,stree,gtree,param_count);
    }
